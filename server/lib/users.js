...
export async function getOrCreatePassenger(tgId){
  await prisma.user.upsert({ where:{tgId:BigInt(tgId)}, update:{}, create:{tgId:BigInt(tgId), role:Role.PASSENGER, passenger:{create:{}}} });
  return prisma.user.findUnique({ where:{tgId:BigInt(tgId)}, include:{ passenger:true } });
}
export async function getOrCreateDriver(tgId){
  await prisma.user.upsert({ where:{tgId:BigInt(tgId)}, update:{}, create:{tgId:BigInt(tgId), role:Role.DRIVER, driver:{create:{}}} });
...
export async function ensureAdmin(tgId){
  ...
  await prisma.user.upsert({ where:{tgId:BigInt(tgId)}, update:{role:Role.ADMIN}, create:{tgId:BigInt(tgId), role:Role.ADMIN, admin:{create:{}}} });
