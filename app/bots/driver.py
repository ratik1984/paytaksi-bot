...
def _driver_blocked(wallet_balance: Decimal) -> bool:
    # "balans -10 olanda sifarish ala bilmesin"
...
async def driver_text(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Session):
    ...
    if step == "car_year":
        ...
        dp.car_year = year
        db.commit()
        await update.message.reply_text("Rəngi seç: aq/qara/qirmizi/boz/mavi/sari/yashil")
...
"
            "Balans: /balance
"
            "Lokasiya göndərmək: canlı location paylaş (mütəmadi yenilə)."
        )
...
async def topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Balans artırma:
"
        "1) /topup_card 10 (Kart2Kart)
"
        "2) /topup_m10 10 (M10)
"
        "Qeyd: Top-up admin təsdiqi ilə balansa düşür."
    )
...
async def _topup_create(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Session, method: TopupMethod):
    ...
    await update.message.reply_text(f"Top-up sorğusu yaradıldı (№{t.id}) — {amount} AZN ({method.value}). Admin təsdiqini gözlə.")
...
async def offer_text(ride: Ride) -> str:
    return (
        f"Yeni sifariş təklifi (№{ride.id})
"
        f"Pick-up: {ride.pickup_address or (ride.pickup_lat + ',' + ride.pickup_lon)}
"
        f"Destination: {ride.dest_address}
"
        f"Məsafə ~ {ride.distance_km} km
"
        f"Təxmini qiymət: {ride.fare_azn} AZN
"
        f"Komissiya: {ride.commission_azn} AZN"
    )

async def driver_router(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Session):
    # ride accept/decline callbacks
    ...
    tg = q.from_user.id
    u = get_or_create_user(db, tg, Role.driver, q.from_user.full_name)
    if action == "accept":
        bal = Decimal(str(u.wallet.balance))
        if _driver_blocked(bal):
            await q.answer("Balans ≤ -10, qəbul edilmir.", show_alert=True)
            return
        ride = db.query(Ride).filter_by(id=ride_id).one_or_none()
        if not ride or ride.status not in {RideStatus.offered, RideStatus.new}:
            await q.answer("Sifariş artıq mövcud deyil.", show_alert=True)
            return
        ride.status = RideStatus.accepted
        ride.driver_user_id = u.id
        db.commit()
        await q.edit_message_text("✅ Sifarişi qəbul etdin.")
    elif action == "decline":
        ride = db.query(Ride).filter_by(id=ride_id).one_or_none()
        if ride and ride.status == RideStatus.offered and ride.driver_user_id == u.id:
            ...
            db.commit()
        await q.edit_message_text("❌ Təklif rədd edildi.")
    await q.answer()
...
def build_handlers():
    return [
        ...
        CommandHandler("topup_card", lambda u,c: None),
        CommandHandler("topup_m10", lambda u,c: None),
        MessageHandler(filters.LOCATION, lambda u,c: None),
