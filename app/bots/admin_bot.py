...
from ..models import DriverProfile, DriverStatus, User, Role, Topup, TopupStatus, Wallet
...
async def pending_topups(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Session):
    items = (
        db.query(Topup)
        .filter(Topup.status == TopupStatus.pending)
...
def build_handlers():
    return [
        ...
        CommandHandler("pending_drivers", lambda u, c: None),  # db-bound in app.main
        CommandHandler("pending_topups", lambda u, c: None),   # db-bound in app.main
